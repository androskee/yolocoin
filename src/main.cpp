...
#if defined(NDEBUG)
# error "Yolocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x005481225d990bbc6dc63a43d1b13a79fcbf539cf0e26028cb09084b4e23f602");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yolocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yolocoin Signed Message:\n";
...
{
    // Yolocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int nTargetTimespan = 600; // Yolocoin: 5 blocks
static const int nTargetSpacing = 2 * 60; // Yolocoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int nActualTimespanMax = nTargetTimespan*1120/1000;
    int nActualTimespanMin = nTargetTimespan*880/1000;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yolocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Yolocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x61c9b1dc188150502205c7d2c7878f63f9493738afc111cde6577a30b235ef99");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "NY Times 10/May/2014 U.S. Officers Kill Armed Civilians in Yemen Capital";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399707828;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086336036;

        if (fTestNet)
        {
            block.nTime    = 1399707828;
            block.nNonce   = 385892772;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb51e637cde10ad8e6c5a462aff625884a0cad69a2bcd5dab91b52a8e514bf5d8"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Yolocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YolocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YolocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YolocoinMiner(CWallet *pwallet)
{
    printf("YolocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yolocoin-miner");
    ...
    try { loop {
        ...
        printf("Running YolocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YolocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YolocoinMiner, pwallet));
}
